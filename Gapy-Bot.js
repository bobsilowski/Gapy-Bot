...
client.on("message", (message) => {
    ...
    const staffargs = message.content.slice(2).trim().split(' ');
    ...
    if (command == "changelog") {
      if (message.member.roles.cache.some(r => r.name == "Admin") || message.member.roles.cache.some(r => r.name == "Manager") || message.member.roles.cache.some(r => r.name == "Owner")) {
          ...
          if (!args[0]) return message.channel.send(":x: Please add a changelog update to this command!")
  ...
  if (message.member.roles.cache.some(r => r.name == "Admin") || message.member.roles.cache.some(r => r.name == "Manager") || message.member.roles.cache.some(r => r.name == "Owner")) {
    ...
    if (command == "suggest" && message.channel.id == "863325297063231518") {
        ...
        let suggestionEmbed = new Discord.MessageEmbed()
            ...
            .setTitle("Suggestion")
            .setAuthor(`From ${message.author.tag}`, message.author.displayAvatarURL({dynamic: true})) 
